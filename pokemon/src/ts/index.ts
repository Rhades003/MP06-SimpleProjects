...
//declaro al sobrecarga para que funcione
...
function addEntities(input1?: string, input2?: string) {
    ...
    if (!input1) {
        ...
        if (inputElement.value?.includes(',')) {
            ...
            let inputArray: string[] = inputElement.value.split(",", 2);
        ...
        else itemList.push(inputElement.value);
    }
    else {
        ...
        if (input1 && input2) pokemonMap.set(input1, input2);

        //si no lo añado en la array de items
        else if (input1) itemList.push(input1);
    }
}
...
function printLists(input: string) {
    ...
    //si el input es pokemon relleno la tabla con dos columnas
        ...
        if (pokemonMap.size > itemList.length) maxLength = pokemonMap.size;
        else maxLength = itemList.length;
        ...
        for (let i = 0; i < maxLength; i++) {
            ...
            if (i < pokemonMap.size) cell1.innerHTML = pokemonArray[0];
            else cell1.innerHTML = "-";


            let cell2: HTMLTableCellElement = document.createElement('td');

            if (i < pokemonMap.size) cell2.innerHTML = pokemonArray[1];
            else cell2.innerHTML = "-";


            let cell3: HTMLTableCellElement = document.createElement('td');

            if (i < itemList.length) cell3.innerHTML = itemList[i];
...
function showContent() {
    ...
    let pokeball: HTMLDivElement = document.getElementById("pokeball") as HTMLDivElement;
